...
-- Update export_all_data to include session_variants
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'session_variants', COALESCE((SELECT jsonb_agg(row_to_json(sv)) FROM session_variants sv WHERE sv.user_id = _uid), '[]'::jsonb)
  ) INTO result;
...
-- Update import_all_data to include session_variants
...
BEGIN
  ...
  -- Import session_variants (after chat_sessions)
  _count := 0;
  FOR _item IN SELECT * FROM jsonb_array_elements(COALESCE(payload->'session_variants', '[]'::jsonb))
  LOOP
    INSERT INTO session_variants (id, user_id, parent_session_id, variant_label, blueprint_content, blueprint_params, blueprint_locked, is_active, pinned, created_at, updated_at)
    VALUES (
      (_item->>'id')::uuid, _uid,
      (_item->>'parent_session_id')::uuid,
      COALESCE(_item->>'variant_label', 'A'),
      COALESCE(_item->>'blueprint_content', ''),
      COALESCE(_item->'blueprint_params', '{}'::jsonb),
      COALESCE((_item->>'blueprint_locked')::boolean, false),
      COALESCE((_item->>'is_active')::boolean, false),
      COALESCE((_item->>'pinned')::boolean, false),
      COALESCE((_item->>'created_at')::timestamptz, now()),
      COALESCE((_item->>'updated_at')::timestamptz, now())
    ) ON CONFLICT (id) DO UPDATE SET
    ...
    _count := _count + 1;
  END LOOP;
  _counts := _counts || jsonb_build_object('session_variants', _count);
