...
-- Migrate saved_suggestions.suggestions from text[] to jsonb
-- Each suggestion will be: {value: string, created_at: string, pinned: boolean, serial: number}

-- First convert existing data
ALTER TABLE public.saved_suggestions 
  ADD COLUMN suggestions_new jsonb DEFAULT '[]'::jsonb;

-- Migrate existing text[] data to jsonb array of objects
...
SET suggestions_new = (
  ...
  FROM unnest(suggestions) WITH ORDINALITY AS t(elem, row_number)
);

-- Drop old column and rename
ALTER TABLE public.saved_suggestions DROP COLUMN suggestions;
ALTER TABLE public.saved_suggestions RENAME COLUMN suggestions_new TO suggestions;

-- Set NOT NULL with default
ALTER TABLE public.saved_suggestions ALTER COLUMN suggestions SET NOT NULL;
ALTER TABLE public.saved_suggestions ALTER COLUMN suggestions SET DEFAULT '[]'::jsonb;

-- Update export function to handle new format
...
-- Update import function for jsonb suggestions
...
BEGIN
  ...
  -- 13. Import saved_suggestions (now jsonb format)
  _count := 0;
  ...
  LOOP
    ...
    VALUES (
      ...
      COALESCE(_item->'suggestions', '[]'::jsonb),
      COALESCE((_item->>'created_at')::timestamptz, now()),
      COALESCE((_item->>'updated_at')::timestamptz, now())
    ) ON CONFLICT (id) DO UPDATE SET
